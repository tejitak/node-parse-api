...
function Parse(options) {
    this._options = {};
    this._options.app_id = options.app_id
    this._options.master_key = options.master_key
    this._options.api_key = options.api_key
    this._options.api_protocol = options.api_protocol === 'http' ? require('http') : require('https')
    this._options.api_host = options.api_host || 'api.parse.com'
    this._options.api_port = options.api_port || 443
    this._options.api_path = options.api_path || '/1'
}

Parse.prototype = {
    // add object to class store
    insert: function (className, object, callback) {
        parseRequest.call(this, 'POST', this._options.api_path + '/classes/' + className, object, callback);
    },

    // add files
    insertFile: function(fileName, data, contentType, callback){
        parseRequest.call(this, 'POST', this._options.api_path + '/files/' + fileName, data, callback, contentType);
    },

    // create a new user
    insertUser: function (data, callback) {
        parseRequest.call(this, 'POST', this._options.api_path + '/users/', data, callback);
    },

    findWithObjectId: function (className, objectId, callback){
        var url = this._options.api_path + '/' + (className === '_User' ? 'users' : 'classes/' + className + '/' + objectId);
    ...
    find: function (className, query, callback) {
        var url = this._options.api_path + '/' + (className === '_User' ? 'users' : 'classes/' + className);
    ...
    batch: function (requests,callback) {
        parseRequest.call(this,'POST',this._options.api_path + '/batch',{requests:requests},callback);
    },

    // user login
    loginUser: function (username, password, callback) {
        parseRequest.call(this, 'GET', this._options.api_path + '/login/?username=' + username + '&password=' + password, null, callback);
    },

    // user logout
    logoutUser: function (sessionToken, callback) {
        parseRequest.call(this, 'POST', this._options.api_path + '/logout', null, callback, null, sessionToken);
    },

    // retrieve current user
    me: function (sessionToken, callback) {
        parseRequest.call(this, 'GET', this._options.api_path + '/users/me', null, callback, null, sessionToken);
    },
    ...
    findFileByUser: function(userId, className, callback) {
        ...
        parseRequest.call(this, 'GET', this._options.api_path + '/classes/' + className + '?' + encodedString, null, callback);
    },
    ...
    insertCustom: function (className, object, callback) {
        parseRequest.call(this, 'POST', this._options.api_path + '/' + className, object, callback);
    },

    // update an object in the class store
    update: function (className, objectId, object, callback) {
        parseRequest.call(this, 'PUT', this._options.api_path + '/classes/' + className + '/' + objectId, object, callback);
    },
    ...
    updateUserEmail: function(objectId, data, sessionToken, callback) {
        ...
        data = { email: data };
        parseRequest.call(this, 'PUT', this._options.api_path + '/users/' + objectId, data, callback, null, sessionToken);
    },
    ...
    passwordReset: function (data, callback) {
        data = { email: data };
        parseRequest.call(this, 'POST', this._options.api_path + '/requestPasswordReset', data, callback);
    },

    // remove an object from the class store
    delete: function (className, objectId, callback) {
        parseRequest.call(this, 'DELETE', this._options.api_path + '/classes/' + className + '/' + objectId, null, callback);
    },
    ...
    deleteUser: function (objectId, sessionToken, callback) {
        ...
        parseRequest.call(this, 'DELETE', this._options.api_path + '/users/' + objectId, null, callback, null, sessionToken);
    },
    ...
    insertInstallationData: function (deviceType, deviceToken, callback) {
        ...
        parseRequest.call(this, 'POST', this._options.api_path + '/installations/', data, callback);
    },
    ...
    updateInstallationDataChannels: function(objectId, channels, callback) {
        parseRequest.call(this, 'PUT', this._options.api_path + '/installations/' + objectId, {
            channels: channels
        }, callback);
    },

    getInstallationData: function(callback) {
        parseRequest.call(this, 'GET', this._options.api_path + '/installations', null, callback);
    },

    getInstallationDataForDeviceToken: function(deviceToken, callback) {
        parseRequest.call(this, 'GET', this._options.api_path + '/installations?where={"deviceToken":"'+deviceToken+'"}', null, callback);
    },

    deleteInstallation: function(objectId, callback) {
      parseRequest.call(this, 'DELETE', this._options.api_path + '/installations/' + objectId, null, callback);
    },
    ...
    countObjects: function (className, query, callback) {
        if (typeof(query) === "function") {
            parseRequest.call(this, 'GET', this._options.api_path + '/classes/' + className, null, query);
        }
        if (typeof(query) === "string") {
            parseRequest.call(this, 'GET', this._options.api_path + '/classes/' + className + '/' + query, null, callback);
        } else {
            parseRequest.call(this, 'GET', this._options.api_path + '/classes/' + className, { where: JSON.stringify(query) }, callback);
        }
    },
    ...
    addRelation: function( relationName, className1, objectId1, className2, objectId2, callback) {
        ...
        parseRequest.call(this,'PUT',this._options.api_path + '/classes/' + className1+'/'+objectId1,data,callback);
    },
    ...
    insertRole: function (data, callback) {
        parseRequest.call(this, 'POST', this._options.api_path + '/roles/', data, callback);
    },

    getRole: function (objectId, callback) {
        parseRequest.call(this, 'GET', this._options.api_path + '/roles/' + objectId, null, callback);
    },

    getRoles: function (params, callback) {
        if (typeof(params) === "function") {
            parseRequest.call(this, 'GET', this._options.api_path + '/roles/', null, params);
        }
        if (typeof(params) === "string") {
            parseRequest.call(this, 'GET', this._options.api_path + '/roles/' + params, null, callback);
        } else {
            params = JSON.stringify(params);
            parseRequest.call(this, 'GET', this._options.api_path + '/roles/', params, callback);
        }
    },

    updateRole: function (objectId, data, callback) {
        parseRequest.call(this, 'PUT', this._options.api_path + '/roles/' + objectId, data, callback);
    },

    deleteRole: function (objectId, callback) {
        parseRequest.call(this, 'DELETE', this._options.api_path + '/roles/' + objectId, callback);
    },

    sendPush: function (data, callback) {
        parseRequest.call(this, 'POST', this._options.api_path + '/push/', data, callback);
    },

    getCurrentSession: function(objectId, callback)
    {
      parseRequest.call(this, 'GET', this._options.api_path + '/sessions/'+ objectId, callback);
    }
...
function parseRequest(method, path, data, callback, contentType, sessionToken) {
    ...
    var options = {
        hostname: this._options.api_host,
        port: this._options.api_port,
        headers: headers,
        path: path,
        method: method
    };

    var req = this._options.api_protocol.request(options, function (res) {
        if (!callback) {
            return;
        }
...
function toDeleteOps(className, objects){
    return objects.map(function(object){
        return {
            method: 'DELETE',
            path: this._options.api_path + '/' + ( className === '_User' ? 'users/' : 'classes/' + className + '/' ) + object.objectId
        };
    });
}
